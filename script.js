...
function updateNavVisibility(sectionId) {
    ...
    // شرط إخفاء السهم: إذا كانت واجهة الترحيب معروضة، أو إذا كنا في قسم login أو grade
    if ((intro && intro.style.display !== "none") || sectionId === "login" || sectionId === "grade") {
        navArrows.classList.add("hidden");
    } else if (sectionId === "grades" || sectionId === "results" || sectionId === "prayerPage") {
        // يظهر فقط في هذه الأقسام
...
window.onload = () => {
 ...
 if(storedUser) {
     ...
     // تأكد من إخفاء السهم عند البداية الجديدة
